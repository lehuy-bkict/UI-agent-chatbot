...
export const BotAvatar = ({ className = "bot-avatar" }) => (
  <svg
    className={className}
...
export const Message = ({ type, content, thinking }) => {
  if (type === 'user') {
    return (
  ...
  return (
    <div className={`message bot-message ${thinking ? 'thinking' : ''}`}>
      <BotAvatar />
      <div className="message-text" style={{ 
        wordBreak: 'normal',
        overflowWrap: 'break-word',
...
export const EmojiPicker = ({ onSelect, onClose }) => {
  ...
  useEffect(() => {
    if (typeof window.EmojiMart !== 'undefined' && pickerRef.current) {
