...
const ChatHeader = ({ onClose }) => (
  <div className="chat-header">
    <div className="header-info">
      ...
      <h2 className="logo-text">Agent Builder Guide Chatbot</h2>
    </div>
...
function App() {
      ...
      setMessages(prev => {
        ...
        if (lastBotMessageIndex !== -1) {
          newMessages[lastBotMessageIndex] = {
            type: 'bot',
            content: hasError ? 'Đã xảy ra lỗi khi xử lý tin nhắn' : text,
            thinking: false,
            error: hasError
          };
        }
  ...
  const handleDeleteChat = () => {
    // Reset messages về trạng thái ban đầu, giữ lại câu chào
    setMessages([
      { type: 'bot', content: 'Hey there 👋 How can I help you today?' }
    ]);
    setMessageInput('');
